...
//
    ...
    KeyFrame::KeyFrame(KeyFrame* kf_to_add, Map* base_map_ptr, KeyFrameDatabase* pKFDB)
    :
            mnFrameId(0), mTimeStamp(0.0),
            mnGridCols(FRAME_GRID_COLS), mnGridRows(FRAME_GRID_ROWS),
            mnMaxX(FRAME_MN_MAX_X), mnMinX(FRAME_MN_MIN_Y), mnMaxY (FRAME_MN_MAX_Y), mnMinY (FRAME_MN_MIN_Y),
    ...
    {
        bool isDebugMode = false;
        mnId = kf_to_add->GetmnId(); // set ID
        nNextId++; // also increment the next id so that we don't have keyframes with the same id
        SetPose( kf_to_add->GetPose() ); // set the pose of the keyframe


        if (isDebugMode)
            cout << "KF [" << GetmnId() << "] adding " << kf_to_add->GetIndexedMapPoints().size() << " matched map points" << endl;
        std::vector <MapPoint*> mps = kf_to_add->GetIndexedMapPoints();
        for (MapPoint * mp : mps)
        {
            if (mp)
                if (!mp->isBad())
                {
                    cv::Mat pos = mp->GetWorldPos();
                    int number_of_observations = mp->nObs;
                    cv::Mat descriptor = mp->GetDescriptor();

                    MapPoint *mp_in_base_map = new MapPoint (pos, this, base_map_ptr, number_of_observations, descriptor);

                    mp_in_base_map->SetMapPointer(base_map_ptr);
                    mvpMapPoints.push_back(mp_in_base_map);
                    base_map_ptr->AddMapPoint(mp_in_base_map);
                }
        }


        if (isDebugMode)
            cout << "The keyframe [" << GetmnId() << "] has " << GetIndexedMapPoints().size() << " map-points" << endl;
...
//        cout << "GetConnectedKeyFrames(): Release lock for " << this->mnId << endl;
    ...
    void KeyFrame::AddMapPoint (MapPoint *pMP)
    {
        static int mp_index = 0;
        mp_index++;
//        unique_lock<mutex> lock(mMutexFeatures);
