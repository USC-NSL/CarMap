...
//            label = 255;
    ...
    MapPoint::MapPoint ( const cv::Mat &newPos, unsigned int kf_id, Map *pMap, int numberOfObservations, cv::Mat desc, unsigned char semanticLabel)
    :
            mnId (kf_id), // mnId in this case will be the ID of the keyframe that this map-point belongs to
            nObs(numberOfObservations),
            mDescriptor (desc),
            label(semanticLabel),
            mpMap(pMap),
...
//                cerr << "Invalid label = " << (int)setLabel << endl;
    ...
    unsigned char MapPoint::GetSemanticLabel() {
        {
            unique_lock<mutex> (mMutexLabel);

            // If the semantic label does not make any sense
            if (((unsigned char) label < 0) || ((unsigned char) label > SEMANTIC_SEGMENTOR_NUMBER_OF_OBJECTS - 1));
                // Do something useful
    ...
    void MapPoint::ReplaceReferenceKeyFrameInBaseMap (KeyFrame* newRefKF)
    {
