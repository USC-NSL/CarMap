...
//
    ...
    vector <MapPoint*> Map::GetNewMapPoints ()
    {
        vector <MapPoint*> new_map_points;
        vector <MapPoint*> all_mps = GetAllMapPoints ();
        // iterate through the whole list of map points and add the new ones to new_map_points
        for ( MapPoint* mp : all_mps)
        {
            if ( mp == NULL)
                continue;
            else if ( mp->isBad() == true )
                continue;
            else if ( mp->GetFromPreviousSession() == true )
                continue;
            else{
                new_map_points.push_back(mp);
                // mark this point as detected now
                mp->SetFromPreviousSession();
            }
        }
        cout << "Number of new map-points = " << new_map_points.size () << " / " << all_mps.size() << endl;
        return new_map_points;
    }

    vector <KeyFrame*> Map::GetNewKeyFrames ()
    {
        vector <KeyFrame*> new_keyframes;
        vector <KeyFrame*> all_keyframes = GetAllKeyFrames();
        // iterate through the whole list of keyframes and add only the ones observed in this session to the new_keyframes list
        for ( KeyFrame* kf : all_keyframes )
        {
            if ( kf == NULL )
                continue;
            else if ( kf->isBad() == true )
                continue;
            else if ( kf->isLoadedFromDisk == true )
                continue;
            else
            {
                new_keyframes.push_back ( kf );
                // mark this as detected or loaded from disk
                kf->isLoadedFromDisk = true;
            }
        }
        cout << "Number of new keyframes = " << new_keyframes.size() << " / " << all_keyframes.size () << endl;
