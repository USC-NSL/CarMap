...
namespace ORB_SLAM2 {
    ...
    System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor,
            ...
            : mSensor(sensor), is_save_map(is_save_map_),
              ...
              upload_map_to_cloud_service (isUploadMap)
    {
...
//        RunLocalBundleAdjustmentOnAllNodes();
              ...
              upload_map_to_cloud_service (isUploadMap)
    {
        ...
        //Upload map on the fly?
        if (isUploadMap == true) {
            cout << "Upload map to cloud service: true" << endl;
            cout << "Establishing connection ...." << endl;
            EstablishConnectionWithServer();
        }
        else
        {
            cout << "CarMap offline mode, not uploading map segment to cloud service" << endl;
        }
...
//        RunLocalBundleAdjustmentOnAllNodes();
    ...
    void System::EstablishConnectionWithServer() {
        ...
        cout << "Establishing connection with CarMap cloud ...." << endl;
        tcpSocket.connect(
        ...
        cout << "Connected to CarMap cloud ....." << endl;


//        float totalData = 0;
//
//        std::time_t previousTime;
//        cout << "Time, Chunk, Total Data" << endl;
//        const int Number_Of_Uploads = 1;
//        for (int counter = 0; counter < Number_Of_Uploads; counter++) {
//
//
//            // read header
//            size_t header;
...
//            buf.commit(header);
//
//            totalData += rc * 0.000001;
//
//            std::time_t result = std::time(nullptr);
//            std::cout << result << ", " << rc * 0.000001 << ", " << totalData << std::endl;
//
//            ORB_SLAM2::Map *map_ptr = new ORB_SLAM2::Map ();
//
//            // deserialize
//            std::istream is(&buf);
//            boost::archive::binary_iarchive ar(is);
//            ar & map_ptr;
//            cout << "Received map over the network ...." << endl;
//
//
//        }
    ...
    ORB_SLAM2::Map* System::GetDiffMap ()
    {
        cout << "Calculating the diff between current session and pre-loaded map ...." << endl;
        ORB_SLAM2::Map* diff_map = new ORB_SLAM2::Map ();
        // get all the new map points that were observed in this session
        vector <MapPoint*> new_mps = GetMap()->GetNewMapPoints();
        vector <KeyFrame*> new_keyframes = GetMap()->GetNewKeyFrames();

        // Add all the new map-points to the diff map
        for ( MapPoint* mp : new_mps )
        {
            // Get mp's meta data
            cv::Mat mp_pos = mp->GetWorldPos();
            int mp_obs = mp->nObs;
            cv::Mat mp_desc = mp->GetDescriptor();
            unsigned char mp_label = mp->GetSemanticLabel();
            unsigned int kf_id = mp->GetReferenceKeyFrame()->GetmnId();

            // Construct a new map-point
            // The mn_id of the map-point is actually the kf_id of its reference keyframe
            MapPoint* mp_to_add = new MapPoint ( mp_pos, kf_id, diff_map, mp_obs, mp_desc, mp_label );

            // Add the keyframe to the diff_map
            diff_map->AddMapPoint (mp_to_add);
        }

        // Add all the new keyframe to the diff map
        for ( KeyFrame* kf : new_keyframes )
        {
            unsigned int kf_id = kf->GetmnId();
            cv::Mat kf_pos = kf->GetPose();
            bool kf_first_connection = kf->GetFirstConnection();
            KeyFrame* kf_parent = kf->GetParent();

            KeyFrame* kf_to_add = new KeyFrame (kf_id, kf_pos, kf_first_connection, kf_parent, diff_map);

            diff_map->AddKeyFrame (kf_to_add);
...
//                if ( AreLabelClassesConsistent ( kf->mvGTLabels [featureCounter], kf->mvLabels[featureCounter] ) )
    ...
    void System::MapUpdate (std::ofstream *e2e_logger, bool reset_mapping)
    {
        // The reset_mapping flag is true if we are sending a new map segment and false if we are just sending a diff
        cout << "Map update ...." << endl;

        static int tx_counter = 0;
        std::chrono::steady_clock::time_point tx_begin, tx_end;
        float tx_map_size;

        if ( reset_mapping == true ) // send the whole map segment
        {
            // Send map over the network
            // Measure TX time by measuring RTT
            cout << "Sending map over the network ..." << endl;
            tx_begin = std::chrono::steady_clock::now();
            tx_map_size = SendMapOverNetwork();
            tx_end = std::chrono::steady_clock::now();
            ResetMapping();
        }
        else // Send only the diff
        {
            // Send only the diff between the current segment and the base map
            // Measure TX time by measuring RTT
            cout << "Sending the diff between the base map and the current segment" << endl;
            tx_begin = std::chrono::steady_clock::now();
            tx_map_size = SendDiffMapOverNetwork ();
            tx_end = std::chrono::steady_clock::now ();
        }


        cout << "Receiving the same map ...." << endl;
        boost::asio::streambuf buf;
        float rt_map_size = ReceiveMap(&tcpSocket, &buf);
        std::chrono::steady_clock::time_point tx_rtt_end = std::chrono::steady_clock::now();
        // RTT calculations
        float tx_rtt_duration = (std::chrono::duration_cast<std::chrono::microseconds>(tx_rtt_end - tx_begin).count()) / 1000000.0f;
        cout << "Map sent = Map received: " << tx_map_size << " = " << rt_map_size << endl;


        // Receive the stitched map over the network
        // Also send the same map back so that the cloud service can measure RTT
        cout << "Waiting to receive map now ...." << endl;
        float rx_map_size = ReceiveMapOverNetwork();
        std::chrono::steady_clock::time_point end = std::chrono::steady_clock::now();
        cout << "Done! Received map over network" << endl;
        //Reconstruct the map's data structures
        float e2e_latency_seconds = (std::chrono::duration_cast<std::chrono::microseconds>(end - tx_begin).count()) / 1000000.0f;
        float tx_seconds = (std::chrono::duration_cast<std::chrono::microseconds>(tx_end - tx_begin).count()) / 1000000.0f;
        float rx_seconds = (std::chrono::duration_cast<std::chrono::microseconds>(end - tx_end).count()) / 1000000.0f;
        std::cout << "CarMap E2E latency = " << e2e_latency_seconds << " " << " seconds" << std::endl;

        // TX_Counter, TX_Time, TX_Map_Size, RX_Time, RX_Map_Size, E2E_Latency
        *e2e_logger << tx_counter++ << ","
                    << ( tx_rtt_duration / 2.0f ) << ","
                    << tx_map_size << ","
//                    << rx_seconds << ","
                    << rx_map_size << ","
                    << e2e_latency_seconds << endl;
    }

    void System::SendMapAndReset (std::ofstream *e2e_logger, bool reset_mapping)
    {
        if (upload_map_to_cloud_service == true)
        {
            MapUpdate(e2e_logger, reset_mapping);
            if (reset_mapping == true)
            {
                cout << "Resetting and starting new mapping session" << endl;
                Reset();
                cout << "Reset!" << endl;
            }
            else
                {
                cout << "Not resetting the map and resuming mapping" << endl;
                mpLocalMapper->Release();
                cout << "Released mapping" << endl;
            }
        }
        else
            cout << "Running in offline mode so cannot upload to cloud service" << endl;
    }
...
//        MajorityVoting ();
        ...
        // Send map over the network and then close down the connection
//        if (upload_map_to_cloud_service)
//            MapUpdate();
//        else
//            cout << "CarMap: No map update" << endl;
...
//        cout << "Total size of map = " << mapFile.size() * 0.000001 << " megabytes" << endl;
    ...
    float System::ReceiveMapOverNetwork ()
    {
        /*

//        string ip_address = "204.57.7.22";
//
...
//        boost::asio::ip::tcp::acceptor acceptor (tcpSocket);
*/
        cout << "Waiting to receive map over network ..." << endl;
        float rx_map_size = 0;
        std::time_t previousTime;
        const int Number_Of_Uploads = 1;
        // read header
        boost::asio::streambuf buf;
        rx_map_size = ReceiveMap(&tcpSocket, &buf);
        // Send the map back because we want to measure RTT at the cloud service
        float tx_map_size = SendMap (&tcpSocket, &buf);
        ...
        ar & map_ptr;
        cout << "Received map over the network ...." << endl;
        cout << "Received map = Sent map: " << rx_map_size << " = " << tx_map_size << endl;

        return rx_map_size;
    }

    float System::ReceiveMap (boost::asio::ip::tcp::socket* carmap_socket, boost::asio::streambuf *buf)
    {
        ...
        buf->commit(header);
        float received_map_size_in_mb = received_map_size * BYTES_TO_MB;
        return received_map_size_in_mb;
    }

    float System::SendMap (boost::asio::ip::tcp::socket* carmap_socket, boost::asio::streambuf* write_buf)
    {
    ...
    void System::SerializeMap (ORB_SLAM2::Map* map_to_serialize, boost::asio::streambuf* write_buf)
    {
    ...
    void System::WaitForMapping ()
    {
        // Wait for mapping to stop
        cout << "Waiting for local mapping to stop ...." << endl;
        mpLocalMapper->RequestStop();
        // Wait until Local Mapping has effectively stopped
        while (!mpLocalMapper->isStopped())
        {
    ...
    float System::SendMapOverNetwork ()
    {
        // Wait for mapping to stop
        WaitForMapping();
        // Serialize binary map file
        boost::asio::streambuf buf;
        SerializeMap ( GetMap(), &buf);
        // Send the serialized data over to the network
        float tx_map_size = SendMap(&tcpSocket, &buf);
        // return the amount of data sent
        return tx_map_size;
    }

    float System::SendDiffMapOverNetwork ()
    {
        cout << "Sending only the diff ...." << endl;
        // Wait for mapping to stop
        WaitForMapping();
        // Serialize binary map file
        boost::asio::streambuf buf;
        Map* diff_map = GetDiffMap();
        SerializeMap ( diff_map, &buf);
        cout << "Number of keyframes in diff map = " << diff_map->GetAllKeyFrames().size() << endl;
        cout << "Number of map points in diff map = " << diff_map->GetAllMapPoints().size() << endl;
        // Send the serialized data over to the network
        float tx_map_size = SendMap(&tcpSocket, &buf);
        // return the amount of data sent
        return tx_map_size;
...
//        std::ofstream out(filename + ".bin", std::ios_base::binary);
        ...
        if (!out) {
            cerr << "Cannot Write to Mapfile at: " << filename << std::endl;
            exit(-1);
        }
...
//                if (counter == 111 || counter == 158 || counter == 222 || counter == 232 || counter == 233 || counter == 683 || counter == 693 || counter == 697 || counter == 698)
//                    continue;
//
//                if (counter == 719 || counter == 721 || counter == 905 || counter == 906 || counter == 907 || counter == 908 || counter == 949 || counter == 950 || counter == 951)
//                    continue;

//                if (counter == 89 || counter == 92 || counter == 97 || counter == 165 || counter == 249 || counter == 253 || counter == 413 || counter == 415 || counter == 417 || counter == 421 || counter == 423)
//                    continue;
